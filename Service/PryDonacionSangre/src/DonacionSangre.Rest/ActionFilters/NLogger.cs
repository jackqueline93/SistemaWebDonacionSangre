...
namespace DonacionSangre.Rest.ActionFilters
{
    public sealed class NLogger : ITraceWriter
    {
        ...
        public void Trace(HttpRequestMessage request, string category, TraceLevel level, Action<TraceRecord> traceAction)
        {
            if (level != TraceLevel.Off)
            {
                ...
                {
                    category = category + Environment.NewLine + "Action Parameters : " + traceAction.Target.ToString(); // .ToJSON();
                }
        ...
        private void Log(TraceRecord record)
        {
            ...
            if (record.Exception != null && !string.IsNullOrWhiteSpace(record.Exception.GetBaseException().Message))
            {
                ...
                else if (exceptionType == typeof(ApiBusinessException))
                {
                    ...
                    if (exception != null)
                    {
                        message.Append("").Append("Error: " + exception.Message + Environment.NewLine);
                        message.Append("").Append("Error Code: " + exception.GetHashCode().ToString() + Environment.NewLine);
                    }
                }
                else
